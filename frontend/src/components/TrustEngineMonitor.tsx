...
// --- Type Definitions to match child component and data sources ---
...
// This matches the props required by TrustEngineMetrics
...
const TrustEngineMonitor = () => {
  ...
  const { data: trustEvents, isLoading: eventsLoading } = useQuery<TrustEvent[]>({
  ...
  const getEventIcon = (delta: number) => {
  ...
  const getEventColor = (eventType: string) => {
    ...
    return colors[eventType] || 'border-gray-400 text-gray-400';
  };
  ...
  const calculateTrustScore = async (agentId: string) => {
    try {
      ...
      setCalculationResults(data as CalculationResults);
      setSelectedAgent(agentId);
      
      await queryClient.invalidateQueries({ queryKey: ['trust-events'] });
      await queryClient.invalidateQueries({ queryKey: ['agents-trust'] });

    } catch (error) {
      console.error('Error calculating trust score:', error);
      toast({
        title: "Error",
        description: "Could not calculate trust score.",
        variant: "destructive",
      });
    }
  };
  ...
  const averageTrustScore = agents && agents.length > 0
    ? agents.reduce((sum, agent) => sum + agent.trust_score, 0) / agents.length
    : 0;
